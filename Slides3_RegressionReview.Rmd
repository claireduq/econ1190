...
\normalsize
Let's draw a line:  $Y=\beta_0+\beta_1X$ and select $\beta_0$ and $\beta_1$ to ``fit'' the data as \textit{closely} as possible.
...
I make a variable (\textit{Male}):\textit{Male}=1 for males, 0 for females. 

I estimate the model: $\text{Earnings}=\beta_0+\beta_1\text{Male}+\epsilon$
\tiny
```{r, echo=TRUE}
...
data$male[data$sex=="female"]<-0

model<-lm(earnings~male, data)
model
```
\normalsize

- $\hat{\beta_0}=16.3$, the estimate for the omitted category (Females)

- $\hat{\beta_1}=3.7$, the estimated difference between Males and the omitted category.

## Reg Review: Categorical variables


For men the variable Male=1, so predicted earnings are
$$
\widehat{\text{Earnings}}=16.3+3.7 (1)=20
$$
For women the variable Male=0, so predicted earnings are
$$
\widehat{\text{Earnings}}=16.3+3.7 (0)=16.3
$$
This simple regression on a categorical variable essentially gives us the mean earnings of males and females.



\tiny
```{r}
gender<-ggplot(data, aes(y=fitted.values(model),x=age,color=sex)) +
  geom_line() + ylim(0,30)+
...
## Reg Review: Categorical variables
Now, I estimate the model: $\text{Earnings}=\beta_0+\beta_1\text{Male}+\beta_2 \text{Age}+\epsilon$
\tiny
```{r, echo=TRUE}
model2<-lm(earnings~male+age, data)
model2
```
\normalsize
...
For males the variable Male=1, so predicted earnings are
$$
\widehat{\text{Earnings}}=8.8+3.8 (1)+0.18 \times \text{Age}=12.6+0.18 \times \text{Age}
$$
For females the variable Male=0, so predicted earnings are
$$
...
\tiny
```{r}
gender2<-ggplot(data, aes(y=fitted.values(model2),x=age,color=sex)) +
  geom_line() + ylim(0,30)+
  labs(x = "Age", y = "Earnings")+
  theme_minimal()
gender2
...
Now, I estimate the model: $\text{Earnings}=\beta_0+\beta_1\text{Male}+\beta_2 \text{Age}+\beta_3 \text{Age}\times\text{Male}+\epsilon$
\tiny
```{r, echo=TRUE}
model3<-lm(earnings~male+age+age*male, data)
model3
```
\normalsize
...
\tiny
```{r}
gender3<-ggplot(data, aes(y=fitted.values(model3),x=age,color=sex)) +
  geom_line() + ylim(0,30)+
  labs(x = "Age", y = "Earnings")+
  theme_minimal()
gender3
