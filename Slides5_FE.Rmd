...
|Individual    |Income      |Schooling   |Woman  |
|--------------|------------|------------|----------|
|1	           |22000       |12          |1         |
|2	           |57000       |16          |1         |
|...	         |...         |...         |...       |
...
|Individual    |Income      |Schooling   |Woman   |Year   |
|--------------|------------|------------|--------|-------|
|1	           |22000       |12          |1       |2001   |
|1	           |23000       |12          |1       |2002   |
|2	           |57000       |16          |1       |2001   |
...
If I have multiple Woman observations and multiple non-Woman observations I can control for the effect of being a woman on wages.  
...
$$
Income_{it}=\beta_0+\beta_1 Schooling_{it}+\beta_2Woman_i+ \beta_3 Married_{it}+\epsilon.
$$
...
|Indiv|Income |School |Woman  |Married |Year   |Indiv1   |Indiv2|...    |IndivN|
|-----|-------|-------|-------|--------|-------|---------|------|-------|------|
...
\footnotesize
$$
Inc_{it}=\beta_0+\beta_1Sch_{it}+\beta_2Wom_i+\beta_3 Mar_{it}+\beta_{a1}Ind1_i+\beta_{a2}Ind2_i+...+\beta_{aN-1}Ind(N-1)_i+\epsilon.
$$
\normalsize
...
- What is the implied assumption if $Wom$ only has an $i$ subscript?

- Why are there only (N-1) individual dummies? 

- Will I be able to estimate the $\beta_2$ on $Wom_i$?
...
\tiny
```{r simfe, echo = TRUE, warning=FALSE, message=FALSE}
...
scores <- dummy_cols(scores, select_columns = "class")
...
scores$classFE<-NA
scores$classFE[scores$class_1==1]<-0
scores$classFE[scores$class_2==1]<-10
scores$classFE[scores$class_3==1]<-(-30)
scores$classFE[scores$class_4==1]<-(-35)


scores$score<-80+15*scores$treat+scores$classFE+scores$error
...
cl_mean<-scores %>%
    group_by(class) %>%
    summarize(Classmean = mean(score, na.rm=TRUE), treatmean=mean(treat, na.rm=TRUE))
...
scores$score2<-80+15*scores$treat2+scores$classFE+scores$error
